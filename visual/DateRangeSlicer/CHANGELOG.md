...
* **预设选中态 checkbox 化**：预设项新增左侧勾选框伪元素（`::before` 方框 + `.active::after` 白色对勾），选中态改为"勾选框填强调色 + 文字高亮"，与值列表完全一致；去掉原来的整行蓝框 + 半透明蓝底 + 整行加粗。
* **输入框默认配色更新**：背景 `#142436` → `#193653`、文字 `#FFFFFF`（用户指定），其余颜色按深蓝暗色主题补齐默认值但全部可改。
* **格式面板专业分组**：重排为 切片器模式 / 切片器标头 / 输入框（原触发器）/ 下拉面板（原面板）/ 值（原列表项）/ 滚动条 六个区域，术语对齐 PBI 原生切片器。
* **capabilities 变更 → 换 GUID**：`selection` 对象新增 18 属性。GUID `DateRangeSlicer20260825010` → `DateRangeSlicer20260825011`，displayName 改为「日期区间切片器 v2.9-全局UI可配置」。旧视觉实例需删除画布上的旧视觉后重新导入。
