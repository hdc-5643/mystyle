...
export class DateRangeSlicer implements IVisual {
    ...
    public update(options: VisualUpdateOptions): void {
        ...
        // Power BI 在切页/刷新/视觉重建后会把已保存的筛选器传回 jsonFilters；
        // 优先恢复用户选择，避免切页后回到默认 min/max。
        // 每次刷新都重新计算数据 min/max，保证边界提示与系统日历可选范围始终准确
        this.resolveRange(dv);

        const filterPresent = !!(options.jsonFilters && options.jsonFilters[0]);

        if (filterPresent) {
            // 切页/刷新后恢复已保存的筛选（可能只含单侧），未含的一侧保持未激活
            this.restoreFilter(options.jsonFilters[0] as any);
            this.updateValueDisplay();
    ...
    public getFormattingModel(): FormattingModel {
        ...
        return { cards: [headerCard, selectionCard, labelsCard] };
    }
